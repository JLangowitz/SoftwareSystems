...
Modified by Josh Langowitz for Software Systems Spring 2014 HW4
*/
...
{
    return h1->hash(h1->key)==(h2->hash(h2->key));
}
...
{
    while(list){
        if (list->key->equal(list->key, key))
            return list->value;
        list = list->next;
    }
    return NULL;
...
/* Returns the bucket that a key would hash into in a Map. */
